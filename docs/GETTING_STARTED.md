...
## พิมพ์สดพร้อมให้ Whisper ฟังเสียงซ้ำ

1. หยุด dictation แล้วเลือก engine เป็น **Apple (on-device)**
2. ปิด **Thai + English: local large-v3 — preview, then type** เพื่อออกจากโหมดรอข้อความสุดท้าย
3. เปิด **Automatic corrections** เพื่อให้ Apple Speech พิมพ์ระหว่างพูด
4. เลือก **Correction: local whisper** เมื่อเตรียม runtime/model ตามหัวข้อถัดไปแล้ว

โหมดนี้พิมพ์ข้อความฉบับร่างลงช่องจริง แล้วใช้ผลจากเสียงแก้คำตามหลัง จึงอาจเห็นคำเปลี่ยนขณะพูด หากช่องไม่รองรับการแก้ช่วงข้อความ ระบบเก็บผลแก้ไว้ให้คัดลอก ไม่ส่ง backspace แบบเดาสุ่ม

## ใช้ในแอปและหน้าต่างอื่น

วางเคอร์เซอร์ในช่องที่ต้องการก่อนเริ่มทุกครั้ง แอปจำ process, หน้าต่าง และช่องข้อความของรอบนั้น แล้วตรวจซ้ำก่อนพิมพ์หรือแก้คำ การย้ายหน้าต่างระหว่างรอผลจะไม่เปลี่ยนปลายทางของเสียงเก่า

บางแอปไม่เปิดเผยช่วง selection แต่ยังระบุช่องและข้อความที่เลือกได้ ระบบใช้ clipboard fallback เมื่อยืนยันว่าไม่มี selection หากไม่มีข้อมูลช่องเลย จะยอมวางเฉพาะหน้าต่างเดิมที่ยังไม่มีการคลิก พิมพ์ หรือสลับแอปตั้งแต่เริ่มบันทึก การเปลี่ยนดังกล่าวทำให้ต้องเริ่มรอบใหม่

ถ้าตรวจปลายทางหรือสิทธิ์ไม่ได้ จะเก็บข้อความใน **Copy last transcript** ข้อผิดพลาดชั่วคราวระหว่างส่ง local transcript จะเก็บส่วนที่ยังไม่พิมพ์ และลองส่งตามลำดับเมื่อผลช่วงถัดไปมาหรือเมื่อระบายรอบบันทึก ไม่พิมพ์ส่วนที่ส่งสำเร็จแล้วซ้ำ

รุ่นนี้ยังเป็น native **macOS** การรองรับหลายหน้าต่างไม่ได้หมายถึงรองรับระบบ Microsoft Windows และไม่รับรองทุกช่องในทุกแอป โดยเฉพาะช่องรหัสผ่าน, secure input และพื้นผิวที่ไม่รับ paste
