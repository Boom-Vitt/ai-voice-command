...
struct FalClient: Sendable {
    ...
    /// The variable we look for inside that file.
    private static let keyName = "FAL_KEY"
    ...
    /// Parses `FAL_KEY=<value>` out of a dotenv-style file.
    ///
    ...
    ///   Never traps, and never returns an empty string.
    static func loadKey(from url: URL) throws -> String {
        ...
        for rawLine in contents.split(whereSeparator: \.isNewline) {
            ...
            guard name == keyName else { continue }
    ...
    func transcribe(wav: Data, keyterms: [String]) async throws -> Result {
        ...
        guard (200...299).contains(status) else {
            throw ClientError.http(status, redacting(Self.snippet(of: data)))
        }
        ...
        do {
            ...
            throw ClientError.decoding(redacting(Self.snippet(of: data)))
        }
    ...
    /// A short, printable prefix of a response body.
    private static func snippet(of data: Data) -> String {
        guard !data.isEmpty else { return "<empty body>" }
        let text = String(decoding: data.prefix(errorBodyPrefix), as: UTF8.self)
            .trimmingCharacters(in: .whitespacesAndNewlines)
        if text.isEmpty { return "<\(data.count) bytes of non-text>" }
        return data.count > errorBodyPrefix ? text + "…" : text
    }
